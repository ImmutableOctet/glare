...
namespace engine
{
	...
	entt::meta_any meta_any_from_string(const util::json& value, bool resolve_symbol=true);

	// Attempts to resolve a native value from a raw string value, using reflection.
	entt::meta_any meta_any_from_string(std::string_view value, bool resolve_symbol = true);
}