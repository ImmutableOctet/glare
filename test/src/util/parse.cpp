...
TEST_CASE("util:parse", "[util]")
{
	SECTION("parse_data_member_reference")
	{
		// Regular access with `::` operator.
		{
		...
		// Command syntax where command syntax is prohibited:
		{
	...
	SECTION("parse_single_argument_command")
	{
		// `if` condition treated as command:
		{
		...
		// Command with parentheses in explicit string content:
		{
			auto [command_name, command_content, trailing_expr, is_string_content] = util::parse_single_argument_command
			(
